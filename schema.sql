...
CREATE VIEW SelectedCandidates AS
SELECT 
    CandidateID,
    JobID,
    COUNT(*) AS RoundsPassed,
...
WHERE SC.JobID IS NULL AND R.PostedDate <= date('now', '-1 month');
...
-- Sample APP
INSERT INTO APP (AppID, AppType, ListPrice, Size, Weight, Style)
VALUES (1001, 'Analytics', 499.99, 'Medium', '2lb', 'Modern');

-- Sample COMPONENT
INSERT INTO COMPONENT (ComponentID, PartName, Description)
VALUES (1001, 'Thermal Sensor', 'Used in Analytics devices');

-- Link COMPONENT to APP
INSERT INTO APPPART (AppID, ComponentID, Quantity)
VALUES (1001, 1001, 2);

-- Sample TECHPARTNER
INSERT INTO TECHPARTNER (TechPartnerID, Name, Address, AccountNo, CreditRating, WebServURL)
VALUES (1001, 'Tech Supplies Ltd.', '789 Circuit Lane', 'TS1001', 88, 'http://techsupplies.com');

-- Link TECHPARTNER to COMPONENT
INSERT INTO TECHPARTNERPART (TechPartnerID, ComponentID, UnitType, UnitCost)
VALUES (1001, 1001, 'Piece', 45.00);

-- Sample LAUNCHPLATFORM (required for SUBSCRIPTION foreign key)
INSERT INTO LAUNCHPLATFORM (SiteID, SiteName, SiteLocation)
VALUES (1, 'Main HQ', 'New York');

-- Sample SUBSCRIPTION (user 2 is a team member, user 1 is a customer)
INSERT INTO SUBSCRIPTION (SubscriptionID, SubscriptionsUserProfileID, CustomerID, AppID, SiteID)
VALUES (1001, 2, 1, 1001, 1);
